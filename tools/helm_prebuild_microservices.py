...
def main():
    ...
    for package in [p for p in microservices if _isdir(microdir, p)]:
        ...
        if package in common_create_keystone_admin:
            pkg_value = merge_dict(pkg_values, values['common-create-keystone-admin'])
        if package in pod_http_termination:
            pkg_value = merge_dict(pkg_values, values['pod-http-termination'])
        if package in stateful_services:
            pkg_value = merge_dict(pkg_values, values['stateful-service'])
        if package in values:
            pkg_value = merge_dict(pkg_values, values[package])
