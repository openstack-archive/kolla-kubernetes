...
cinder_volume=$(docker ps -a | grep cinder-volume\:3.0.1 | grep kolla_start | awk '{print $1}')
cinder_log=$(docker inspect $cinder_volume | grep "Source" | grep kolla-logs | awk '{print $2}')
cinder_log_size=${#cinder_log}
cinder_log_path=${cinder_log:1:cinder_log_size-3}
cat $cinder_log_path/cinder/cinder-volume.log >> $WORKSPACE/logs/cinder-volume.log
