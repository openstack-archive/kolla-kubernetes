...
helm dep up
cd ../..
S="adminSecret.key=$key,userSecret.name=ceph-kolla,adminSecret.key=$key"
S="$S,monitors[0]=$IP,args={-v,"10"},pool=kollavolumes,userSecret.user=kolla,userSecret.manage=false"
helm install stable/ceph-provisioner --name ceph-provisioner \
  --namespace kube-system --set "$S" \
  --values <(echo 'nodeSelector:'; echo '  kolla_controller: "true"')
cd ..
