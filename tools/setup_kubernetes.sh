...
apt-get update
apt-get install -y docker.io kubelet kubeadm-1.6.0-0.x86_64 kubectl kubernetes-cni
EOF
fi
cat >> /tmp/setup.$$ <<"EOF"
...
kubeadm init --skip-preflight-checks --service-cidr 172.16.128.0/24 --apiserver-advertise-address $(cat /etc/nodepool/primary_node_private) | tee /tmp/kubeout
