...
EOEF
yum install -y docker kubelet kubeadm kubectl kubernetes-cni ebtables bzip2
...
apt-get update
apt-get install -y docker.io kubelet kubeadm kubectl kubernetes-cni bzip2
EOF
fi
cat >> /tmp/setup.$$ <<"EOF"
...
if [ -d "$WORKSPACE/DOWNLOAD_CONTAINERS" ]; then
    ls "$WORKSPACE/DOWNLOAD_CONTAINERS"/*.bz2 | sudo xargs -i docker load -i {}
    rm -f "$WORKSPACE/DOWNLOAD_CONTAINERS"/*.bz2
fi
sudo docker images
