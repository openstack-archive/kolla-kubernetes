...
EOEF
yum install -y docker kubeadm-1.6.0-0.x86_64 kubelet kubectl kubernetes-cni ebtables iproute
sed -i 's|KUBELET_KUBECONFIG_ARGS=|KUBELET_KUBECONFIG_ARGS=--cgroup-driver=systemd --enable-cri=false |g' \
        /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
cp /etc/systemd/system/kubelet.service.d/10-kubeadm.conf /tmp/10-kubeadm.conf
...
/bin/cp -f /tmp/10-kubeadm.conf /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
