...
#
cluster_cidr=$(sudo bash -c 'grep cluster-cidr /etc/kubernetes/manifests/kube-controller-manager.yaml')
    if [ "x$cluster_cidr" == "x" ]; then
       sudo sed -i '|--kubeconfig|a     - --allocate-node-cidrs=true|' /etc/kubernetes/manifests/kube-controller-manager.yam
       sudo sed -i '|--kubeconfig|a     - --cluster-cidr=10.244.0.0/16|' /etc/kubernetes/manifests/kube-controller-manager.yam
    fi
