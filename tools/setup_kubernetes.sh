...
kubeadm init --skip-preflight-checks --service-cidr 172.16.128.0/24 --api-advertise-addresses $(cat /etc/nodepool/node_private) | tee /tmp/kubeout
...
EOF
timeout 5m sudo bash /tmp/setup.$$
sudo docker ps -a
