...
if [ "$1" == "master" ]; then
    ...
    curl http://storage.googleapis.com/kubernetes-helm/helm-v2.0.0-beta.2-linux-amd64.tar.gz | sudo tar --strip-components 1 -C /usr/bin linux-amd64/helm -zxf -
    mkdir -p ~/.kube
    ln -s /etc/kubernetes/kubelet.conf ~/.kube/config
    helm init
fi
