...
kubeadm init --skip-preflight --service-cidr 172.16.128.0/24 --api-advertise-addresses $(cat /etc/nodepool/primary_node_private) | tee /tmp/kubeout
...
EOF
    ...
    cat >> /tmp/setup.$$ <<EOF
kubeadm join --token "$2" "$3" --skip-preflight
EOF
fi
cat >> /tmp/setup.$$ <<"EOF"
