...
then
  echo "Please provide the following Information to the script:
       \$1 for kube proxy IP, i.e. management nic IP like 10.10.10.100
       \$2 for tunnel_interface, i.e. ens160
       \$3 for ext interface, i.e. ens192
       \$4 for keepalived VIP, i.e. unused IP on management NIC subnet like 10.10.10.254
...
helm install /opt/kolla-kubernetes/helm/service/openvswitch --namespace kolla --name openvswitch --values /opt/cloud.yaml

# Ensure that OVS pod is up before proceeding
cd /opt/kolla-kubernetes && tools/wait_for_pods.sh
...
helm install /opt/kolla-kubernetes/helm/microservice/keepalived-daemonset --namespace kolla --name keepalived-daemonset --values /opt/cloud.yaml
...
helm install /opt/kolla-kubernetes/helm/service/mariadb --namespace kolla --name mariadb --values /opt/cloud.yaml

# Ensure that MariaDB pod is up before proceeding
cd /opt/kolla-kubernetes && tools/wait_for_pods.sh

helm install /opt/kolla-kubernetes/helm/service/rabbitmq --namespace kolla --name rabbitmq --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/memcached --namespace kolla --name memcached --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/keystone --namespace kolla --name keystone --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/glance --namespace kolla --name glance --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/cinder-control --namespace kolla --name cinder-control --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/cinder-volume-lvm --namespace kolla --name cinder-volume-lvm --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/horizon --namespace kolla --name horizon --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/neutron --namespace kolla --name neutron --values /opt/cloud.yaml

# Ensure that Neutron pod is up before proceeding
cd /opt/kolla-kubernetes && tools/wait_for_pods.sh

helm install /opt/kolla-kubernetes/helm/service/nova-control --namespace kolla --name nova-control --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/service/nova-compute --namespace kolla --name nova-compute --values /opt/cloud.yaml

# Ensure that Nova Compute pod is up before proceeding
cd /opt/kolla-kubernetes && tools/wait_for_pods.sh

helm install /opt/kolla-kubernetes/helm/microservice/nova-cell0-create-db-job --namespace kolla --name nova-cell0-create-db-job --values /opt/cloud.yaml
helm install /opt/kolla-kubernetes/helm/microservice/nova-api-create-simple-cell-job --namespace kolla --name nova-api-create-simple-cell --values /opt/cloud.yaml
