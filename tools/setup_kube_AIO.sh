...
## $4 will be the keepalived VIP and $5 will be the subnet size of the keepalived VIP network
...
if [ "$#" -ne 5 ]
then
  echo "Please provide the following Information to the script: \$1 for kube proxy IP, \$2 for tunnel_interface, \$3 for ext interface, \$4 for keepalived VIP, \$5 for subnet size of management network"
  exit 1
fi
...
url="$url/k8s-install/$canal_version/canal.yaml"

curl "$url" -o /tmp/canal.yaml

url="https://raw.githubusercontent.com/projectcalico/canal/master"
url="$url/k8s-install/$canal_version/rbac.yaml"
