#!/bin/bash
[ ! -d ../kolla-ansible ] && pushd .. && git clone https://github.com/openstack/kolla-ansible && cp -ar kolla-ansible/etc/kolla/ /etc/kolla && pip install -U kolla-ansible/ && popd
...
grep orchestration_engine ../kolla-ansible/etc/kolla/globals.yml || echo orchestration_engine: KUBERNETES >> /etc/kolla/globals.yml
