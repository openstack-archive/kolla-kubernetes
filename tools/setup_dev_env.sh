...
ceph_startup () {
    ...
    kubectl create -f /tmp/kube.yaml
    python kolla-kubernetes/tools/wait_for_pods.py ceph-bootstrap-initial-mon succeeded
    ...
    sed -i "s|kubernetes.io/hostname: kube2|kubernetes.io/hostname: 172.16.35.12|g"  /tmp/kube.yaml
    kubectl create -f /tmp/kube.yaml
    python kolla-kubernetes/tools/wait_for_pods.py ceph-mon running
    ...
    kubectl create -f /tmp/kube.yaml
    python kolla-kubernetes/tools/wait_for_pods.py ceph-bootstrap-osd0 succeeded

    kollakube template pod ceph-bootstrap-osd1 > /tmp/kube.yaml
    sed -i "s|kubernetes.io/hostname: kube2|kubernetes.io/hostname: 172.16.35.12|g"  /tmp/kube.yaml
    sed -i "s|loop1|loop3|g"  /tmp/kube.yaml
    kubectl create -f /tmp/kube.yaml
    python kolla-kubernetes/tools/wait_for_pods.py ceph-bootstrap-osd1 succeeded
    ...
    sed -i "s|loop0|loop2|g"  /tmp/kube.yaml
    kubectl create -f /tmp/kube.yaml
    python kolla-kubernetes/tools/wait_for_pods.py ceph-osd0 running

    kollakube template pod ceph-osd1 > /tmp/kube.yaml
    sed -i "s|kubernetes.io/hostname: kube2|kubernetes.io/hostname: 172.16.35.12|g"  /tmp/kube.yaml
    sed -i "s|loop1|loop3|g"  /tmp/kube.yaml
    kubectl create -f /tmp/kube.yaml
    python kolla-kubernetes/tools/wait_for_pods.py ceph-osd1 running
    ...
    kubectl create -f /tmp/kube.yaml
    python kolla-kubernetes/tools/wait_for_pods.py ceph-admin,ceph-rbd running
    ...
    kolla-kubernetes/tools/setup_rbd_volumes.sh --yes-i-really-really-mean-it
}
...
helm_setup () {
    ...
    # TODO -- wait for tiller to be back up and running
    # for some reason the service is still coming up as 2.1.0 not 2.1.3
...
# TODO -- after this run ceph_service_workflow.sh
ceph_setup
config_setup
k8s_setup
ceph_startup
