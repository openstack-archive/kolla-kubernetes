...
if [ "x$BRANCH" == "xt" ]; then
    ...
    sed -i 's/docker_registry:.*/docker_registry: "127.0.0.1"/g' tests/conf/ceph-all-in-one/kolla_config
    sed -i 's/docker_namespace:.*/docker_namespace: "lokolla"/g' tests/conf/ceph-all-in-one/kolla_config
