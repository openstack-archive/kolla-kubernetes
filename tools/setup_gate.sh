...
function install_wget {
    ...
    if [ "$DISTRO == "centos" -o "$DISTRO="oraclelinux" ]; then
        sudo yum -y install wget docker bzip2
    else
        sudo apt-get -y install wget docker.io bzip2
   fi
}
...
function prepare_images {
    ...
    # TODO(sdake): Ccross-repo depends-on is probably broken
...
# BRANCH=$(echo "$ZUUL_BRANCH" | cut -d/ -f2
        ...
        sudo tar xjf "/tmp/$FILENAME" -C /tmp/kolla_registry
    fi
...
# TODO(sdake): I don't understand the purpose of UPLOAD_CONTAINERS
#if [ "x$PIPELINE" == "xperiodic" ]; then
...
# TODO(sdake) pretty sure this is no longer needed tests/bin/build_docker_images.sh $WORKSPACE/logs $DISTRO $TYPE $CONFIG $BRANCH $PIPELINE
