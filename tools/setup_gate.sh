...
cat tests/conf/ceph-all-in-one/kolla_kubernetes_config >> etc/kolla-kubernetes/kolla-kubernetes.yml
sed -i "s/initial_mon:.*/initial_mon: $NODE/" etc/kolla-kubernetes/kolla-kubernetes.yml

kubectl label node $NODE kolla_controller=true
kubectl label node $NODE kolla_compute=true

sudo more /etc/kubernetes/manifests/*
exit -1
