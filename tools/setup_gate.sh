...
function run_gate {
    ...
    if [[ $GATE_TYPE == "ansible" ]]; then
       ...
       ls $KOLLA_KUBERNETES_ANSIBLE_DIR
       ansible-playbook -i $KOLLA_KUBERNETES_ANSIBLE_DIR/inventory/all-in-one -e @/etc/kolla-kubernetes/kolla-kubernetes.yml $KOLLA_KUBERNETES_ANSIBLE_DIR/site.yml
    else
       create_configmaps
       create_secrets
       ceph_setup
