...
ssh_vm {
    sshpass -p 'cubswin:)' ssh -o UserKnownHostsFile=/dev/null -o \
        StrictHostKeyChecking=no cirros@$1 /bin/sh -c "$2"
}
...
#FIXME testing
#FIXME switch disk backing back to rbd
#echo testing
...
TESTSTR=$(uuidgen)
ssh_vm $FIP "mkdir /tmp/mnt"
ssh_vm $FIP "sudo /sbin/mkfs -t ext2 /dev/vdb"
ssh_vm $FIP "sudo /bin/sh -c 'echo $TESTSTR > /tmp/mnt/test.txt'"
ssh_vm $FIP "sudo umount /dev/vdb"
...
ssh_vm $FIP2 "mkdir /tmp/mnt"
ssh_vm $FIP2 "sudo mount /dev/vdb /tmp/mnt"
ssh_vm $FIP2 "cat /tmp/mnt"
