...
function trap_error {
    ...
    kubectl exec ceph-admin -c main --namespace=kolla -- /bin/bash -c "$str"
    str="cat /var/log/ceph/*.log"
    kubectl exec ceph-osd -c main --namespace=kolla -- /bin/bash -c "$str" \
...
for x in kollavolumes; do
    ...
    -c "ceph osd pool create $x 32; ceph osd pool set $x size 1; ceph osd pool set $x min_size 1"
done
...
for volume in mariadb rabbitmq; do
    str='DEV=$(rbd map --pool kollavolumes '$volume'); mkfs.ext4 $DEV;'
