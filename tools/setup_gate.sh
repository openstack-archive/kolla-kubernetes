...
function trap_error {
    ...
    kubectl get pods --all-namespaces -o json | jq -r \
        ...
        echo $NAME | grep neutron-openvswitch-agent
        [ $? -eq 0 ] && kubectl exec $NAME -c main --namespace $NAMESPACE -- /bin/bash -c "ls -al /var/log/kolla/neutron/neutron-openvswitch-agent.log"> \
            $WORKSPACE/logs/neutron-openvswitch-agent.txt
        echo $NAME | grep libvirt
        [ $? -eq 0 ] && kubectl exec $NAME -c main --namespace $NAMESPACE -- /bin/bash -c "cat /var/log/kolla/libvirt/libvirtd.log"> \
            $WORKSPACE/logs/libvirtd.txt
    ...
    mkdir -p $WORKSPACE/logs/kubelet-logs
    sudo find /var/lib/kubelet/pods/ -type f | grep kubernetes.io~empty-dir | while read line; do \
        sudo chmod 666 $line
        sudo cp -a $line $WORKSPACE/logs/kubelet-logs
    done
    exit -1
}
...
ls /etc/kolla/*/*.conf | egrep 'cinder' | while \
    read line; do
    crudini --set $line DEFAULT use_stderr True
done
...
sshpass -p 'cubswin:)' ssh -o UserKnownHostsFile=/dev/null -o \
    StrictHostKeyChecking=no cirros@$FIP ls -l /dev/vdb
