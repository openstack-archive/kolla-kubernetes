...
sudo iptables-save
DEVNUM="$(ip a | sed '/^[^1-9]/d;' | awk '{print $2}' | sed 's/://' | grep -v '^lo$' | wc -l)"
if [ "$DEVNUM" -ne 1 ]; then
    echo "More then one network device found. No idea how to fix iptables."
    exit -1
fi

DEV="$(ip a | sed '/^[^1-9]/d;' | awk '{print $2}' | sed 's/://' | grep -v '^lo$')"

sudo iptables -I INPUT 1 '!' -i $DEV -j ACCEPT
...
tools/setup-resolv-conf.sh --partial-async
sleep 1
