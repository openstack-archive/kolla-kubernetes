...
TESTSTR=$(uuidgen)
str="mkdir /tmp/mnt; sudo /sbin/mkfs -t ext2 /dev/vdb;"
str="$str sudo mount /dev/vdb /tmp/mnt;"
str="$str sudo /bin/sh -c 'echo $TESTSTR > /tmp/mnt/test.txt';"
str="$str sudo umount /dev/vdb;"
sshpass -p 'cubswin:)' ssh -o UserKnownHostsFile=/dev/null -o \
    StrictHostKeyChecking=no cirros@$FIP /bin/sh -c "$str"
