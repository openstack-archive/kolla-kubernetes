...
sudo iptables-save
#FIXME I think there is an unexpected iptables setup... try clearing it.
...
mkdir -p $WORKSPACE/logs
kubectl get jobs --namespace=kolla -o json > $WORKSPACE/logs/j.json
...
kollakube res create bootstrap glance-create-db \
    glance-manage-db nova-create-api-db nova-create-db \
    neutron-create-db neutron-manage-db cinder-create-db \
    cinder-manage-db

#FIXME testing a theory about reformatting yaml and auth bad escaping
for x in glance-endpoints nova-create-endpoints neutron-endpoints \
    cinder-create-endpoints; do
    kollakube res create bootstrap $x
done
...
kollakube res create pod openvswitch-ovsdb-network \
    openvswitch-vswitchd-network neutron-openvswitch-agent-network \
    neutron-dhcp-agent neutron-metadata-agent-network neutron-l3-agent-network
