...
function run_gate {
    ...
    if [[ $GATE_TYPE == "ansible" ]]; then
       setup_ansible
       ANSIBLE_DIR=workflows/ansible
       ansible-playbook -i $ANSIBLE_DIR/inventory/all-in-one -e @/etc/kolla-kubernetes/kolla-kubernetes.yml $ANSIBLE_DIR/site.yml
    else
       create_configmaps
       create_secrets
       ceph_setup
...
FIP1=$(openstack floating ip create external -f value -c floating_ip_address)
FIP2=$(openstack floating ip create external -f value -c floating_ip_address)

openstack server add floating ip test1 $FIP1
openstack server add floating ip test2 $FIP2

echo Creating Volume...
openstack volume create --size 1 test1
wait_for_cinder test1 creating
openstack server add volume test1 test1

openstack server list

wait_for_vm_ssh $FIP1

sshpass -p 'cubswin:)' ssh -o UserKnownHostsFile=/dev/null -o \
    StrictHostKeyChecking=no cirros@$FIP1 curl 169.254.169.254

sshpass -p 'cubswin:)' ssh -o UserKnownHostsFile=/dev/null -o \
    StrictHostKeyChecking=no cirros@$FIP1 ping -c 4 $FIP2
...
scp_to_vm $FIP1 /tmp/$$ /tmp/script
ssh_to_vm $FIP1 "/tmp/script"
