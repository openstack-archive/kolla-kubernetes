...
registry_filename=$(registry_file $BASE_DISTRO $INSTALL_TYPE)
...
if [ "x$4" == "xceph-multi" ]; then
    ...
    cat /etc/nodepool/sub_nodes_private | while read line; do
        ...
        scp tools/setup_registry.shh $line:
        ...
        ssh $line bash setup_kubernetes.sh slave $BASE_DISTRO $INSTALL_TYPE "$(cat /etc/kubernetes/token.txt)" "$(cat /etc/kubernetes/ip.txt)"
        ssh $line rm -f $registry_filename
        set +xe
        count=0
        while true; do
