...
docker stop $(sudo docker ps -a | grep \
    gcr.io/google_containers/kube-apiserver-amd64 | awk '{print $1}')
systemctl start kubelet
EOF
...
TOOLBOX=$(kollakube tmpl create bootstrap neutron-create-db -o json | jq -r '.spec.template.spec.containers[0].image')
docker pull $TOOLBOX > /dev/null
