...
function trap_error {
    ...
    POD=$(kubectl get pods --namespace $1 -o json | jq -r \
        '.items[].metadata.name' | grep neutron-openvswitch)
    str="cat /var/log/kolla/*/*"
...
sshpass -p 'cubswin;)' ssh -o UserKnownHostsFile=/dev/null -o \
    StrictHostKeyChecking=no cirros@$1 curl 169.254.169.254

sshpass -p 'cubswin;)' ssh -o UserKnownHostsFile=/dev/null -o \
    StrictHostKeyChecking=no cirros@$1 ping -c 4 $FIP2
