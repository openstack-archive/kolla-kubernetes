...
COUNT=0
while true; do
    kubectl get pods --all-namespaces -o json | jq -r '.items[].status.phase' \
        | grep Pending && PENDING=True || PENDING=False
    [ $PENDING == "False" ] && break || true
    sleep 1
    COUNT=$((COUNT+1))
    [ $COUNT -gt 120 ] && echo docker failed to starrt. && exit -1
done
