...
for service in glance keystone cinderv2 nova cinder neutron; do
    line=$(openstack catalog show $service -c endpoints -f value)
    echo $line | grep 'public[^:]*:' > /dev/null
    echo $line | grep 'internal[^:]*:' > /dev/null
    echo $line | grep 'admin[^:]*:' > /dev/null
done
