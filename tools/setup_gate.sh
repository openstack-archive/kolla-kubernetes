...
TESTSTR=$(uuidgen)
str="mkdir -p /tmp/mnt; mkfs.ext4 /dev/vdb; mount /dev/vdb /tmp/mnt;"
str="$str echo $TESTSTR > /tmp/mnt/test.txt; umount /dev/vdb1"
sshpass -p 'cubswin:)' ssh -o UserKnownHostsFile=/dev/null -o \
    StrictHostKeyChecking=no cirros@$FIP /bin/bash -c "$str"
