...
function wait_for_pods {
    ...
    while true; do
        kubectl get pods --all-namespaces -o json | jq -r \
            '.items[].status.phase' | grep Pending && PENDING=True || \
            PENDING=False
        query='.items[]|select(.status.phase=="Running")'
        query="$query|.status.containerStatuses[].ready"
        kubectl get pods --all-namespaces -o json | jq -r "$query" | \
            grep false && READY="False" || READY="True"
        [ $PENDING == "False" -a $READY == "True" ] && break || true
        sleep 1
        COUNT=$((COUNT+1))
        [ $COUNT -gt 120 ] && echo containers failed to start. && exit -1
    done
}
