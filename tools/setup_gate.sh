...
function wait_for_ceph_bootstrap {
    ...
    while true; do
        kubectl get pods --namespace=$1 | grep ceph-osd && PENDING=True || \
            PENDING=False
        [ $PENDING == "False" ] && break
        sleep 1
        now=$(date +%s)
...
function trap_error {
    ...
    POD=$(kubectl get pods --namespace $1 -o json | jq -r \
        '.items[].metadata.name' | grep neutron-openvswitch-agent)
    str="cat /var/log/kolla/*/* || cat /var/log/neutron/*"
    kubectl exec -i $POD -c main /bin/bash -c "$str" > \
        $WORKSPACE/logs/neutron-openvswitch-agent.txt
    POD=$(kubectl get pods --namespace $1 -o json | jq -r \
        '.items[].metadata.name' | grep cinder-api)
    str="cat /var/log/kolla/*/* || cat /var/log/cinder/*"
    kubectl exec -i $POD -c main /bin/bash -c "$str" > \
        $WORKSPACE/logs/cinder-api.txt
