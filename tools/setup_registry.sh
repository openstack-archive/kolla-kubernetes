...
ls -l
ls -l $DIR/../
helm install $DIR/../helm/microservice/registry-deployment --debug --dry-run \
             --namespace kolla --name registry \
             --set initial_load=true --set node_port=30401 \
             --set distro=$Distro --set type=$Type \
             --values $DIR/../helm/microservice/registry-deployment/values.yaml
#
#
#

helm install $DIR/../helm/microservice/registry-deployment --debug \
             --namespace kolla --name registry \
             --set initial_load=true --set node_port=30401 \
             --set distro=$Distro --set type=$Type \
             --values $DIR/../helm/microservice/registry-deployment/values.yaml
