...
def _build_bootstraper(working_dir, service_name, variables=None):
    for filename in service_definition.find_bootstrap_files(service_name):
        ...
        with open(os.path.join(working_dir, proj_filename), 'w') as f:
            LOG.debug('_build_bootstraper : file : %s' %
...
def setup_service(service_name, variables=None):
    working_dir = _create_working_directory('bootstrap')
    _build_bootstraper(working_dir, service_name, variables=variables)
