...
To enable the proper cgroup driver and disable CRI::
    ...
    sudo sed -i 's|KUBELET_KUBECONFIG_ARGS=|KUBELET_KUBECONFIG_ARGS=--cgroup-driver=systemd --enable-cri=false |g' /etc/systemd/system/kubelet.service.d/10-kubeadm.conf

Setup the DNS server with the service CIDR::

    sudo sed -i 's/10.96.0.10/10.3.1.10/g' /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
...
Deploy Kubernetes with kubeadm::
    ...
    sudo kubeadm init --pod-network-cidr=10.1.0.0/16 --service-cidr=10.3.1.0/24
...
This should return a nslookup result without error::
    ...
    # nslookup kubernetes
    Server:    10.3.1.10
    Address 1: 10.3.1.10 kube-dns.kube-system.svc.cluster.local

    Name:      kubernetes
    Address 1: 10.3.1.1 kubernetes.default.svc.cluster.local
...
Replace all occurances of 192.168.7.101 with your management interface nic (e.g. eth0)::
...
.. note::
   ...
   Some of the above cloud.yaml files may be customized with the following items:
