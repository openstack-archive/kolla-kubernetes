...
Turn off firewalld::
    ...
    sudo systemctl disable fireewalld
...
To enable Kubernetes to bind to port 8080 as expected by kubectl::

    sudo sed -i 's/insecure-port=0/insecure-port=8080/' /etc/kubernetes/manifests/kube-apiserver.yaml
...
Deploy the canal CNI driver::
    ...
    sudo sed -i "s@192.168.0.0/16@10.0.1.0/16@" canal.yaml
