...
To enable Kubernetes to bind to port 8080 as expected by kubectl::
    ...
    sudo sed -i 's/insecure-port=0/insecure-port=8080/'

To enable the proper cgroup driver and disable CRI::

    sed -i 's|KUBELET_KUBECONFIG_ARGS=|KUBELET_KUBECONFIG_ARGS=--cgroup-driver=systemd --enable-cri=false |g' /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
