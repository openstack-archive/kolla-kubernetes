...
This guide specifies how to deploy kolla-kubernetes within a
multi-node Kubernetes cluster.  It will guide you through all of the
dependencies required to deploy Horizon, the Openstack admin
...
You can also destroy *all* kubernetes nodes, then bring some back, and
...
multi-node cluster.  A few sleeps are added below to allow for
services to bootsrap and startup.  In the future, a workflow engine
will take care of this.  The section below should be cut and pasteable
if you dare.
...
::
  ...
  kolla-kubernetes resource delete bootstrap mariadb  # workaround
...
Deleting all resources is exactly the creation steps in reverse.
