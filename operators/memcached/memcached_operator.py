...
def install_microservice(name, type, operator):

    microservice_url = "http://helm-repo:{}/{}-{}-{}.tgz". \
        format(helm_port, name, type,
               operator.version)

    command_line = 'helm install {} --dry-run --debug --namespace {} \
                    --name {} \
                    --set "port={},element_name={},operator_label={}"' \
                    .format(microservice_url,
                            operator.namespace,
                            name + '-' +
                            operator.label,
                            operator.port,
...
def check_object_status(cluster, operator, object_kind,
    ...
    r_str = build_api_request(cluster, operator, object_kind)
    print(r_str)
    times = timeout // 10
    i = 0
    while (i < times):
        data = json.loads(get_request(cluster, r_str))
...
def service_deployment(cluster, operator):
    ...
    if check_object_status(cluster, operator, object_kind='services',
                           name='memcached', timeout=60) is False:
        return False
    else:
