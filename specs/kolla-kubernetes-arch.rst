...
disaster recovery.  Entrypoints are used for workflow management.
  ...
  - If workflow logic is put into the containers, kolla-kubernetes will be in
    the situation where kolla was 2 years ago with everything resolved at the
    container level.  It failed because Kolla went way too far trying to achieve
    maximum usability, which caused it to lack the complexity and flexibility
    required for an OpenStack deployment.
  ...
  - The visibility and consumability are difficult because like Kolla learned
    from ansible, deploying logic from the client side is more effective then
    having logic run server side. Operators are the client side controllers,
    while entrypoints are run in the containers (server side). Kolla has shown
...
use layer 3 for upgrades or other lifecycle operations.
  ...
  |      Service        |
  | Kubernetes Operator | layer 5 - Executes Helm packages for a service.
  |_________  __________|
            ||
   _________\/__________
  |                     |
  |    Helm Package     | layer 4 - Organizes the Kubernetes resources for each
  |_________  __________|           pod
            ||
   _________\/__________
  |                     |
  | Kubernetes resource | layer 3 - Deploys pods/deployments/replication-
  |_________  __________|           controllers/daemon-sets for an OpenStack
            ||                      service
   _________\/__________
  |   Dependency Init   |
  |      Container      | layer 2 - Checks to be sure service dependencies are
  |_________  __________|           met before starting the service
            ||
   _________\/__________
  |                     |
...
Layer 6 - The OpenStack Kubernetes Operator will look at the config files and
...
Layer 5 - The Glance Kubernetes Operator will gather the config data placed in
...
Layer 4 - The Glance Helm package will run the glance-api and glance-registry
          pods based on the Glance kolla-kubernetes templates.

Layer 3 - The Kubernetes pods run glance-api and glance-registry as Deployments.
          The templates will map the configmaps to the location Kolla expects
          config files to appear.

Layer 2 - The Dependency Init Container checks to be sure glance-api and
