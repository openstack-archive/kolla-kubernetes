...
One of concept of dealing with initial deployment as well as upgrades was
dependency init container. This is variation of kubernetes-entrypoint mechanism.
This model of deployment consists of *optional* lock mechanism using init
containers that will run kubernetes entrypoint inside, blocking pod deployment
until all dependencies are met. Dependencies will be generated by package
manager (for example helm). Dependency can be any resource in kubernetes, like
pod in ready state or job completed.

Example of dependency list for keystone pod
-------------------------------------------

Init container for keystone pod would be blocked until these dependencies are
met:
- mariadb pod ready
- keystone bootstrap job done
- keystone configmap present

After these conditions are met init container is marked as ready and keystone
pod can be deployed.
