...
to handle running a series of tasks in a certain order. To simulate a workflow,
it requires logic to be added into pods so that services will be cluster aware
...
Operator.
  ...
  - This option provides a high amount of granularity with every service being
    ...
    being more granular.  More layers will add flexibility for the user at the
    cost of complexity.
...
building blocks to assemble a real world deployment. The user experiece is far
