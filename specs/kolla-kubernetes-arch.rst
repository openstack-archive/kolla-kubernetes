...
**Kolla-kubernetes will be using option 2 as the model for Kubernetes Operators**
Option 2:
...
and wanted to run Glance, this is what would occur at each layer:
  ...
  User  - The user creates a custom config file for glance-api and saves it in
          ...
          customized config file for glance-api. Glance-registry will use the
          default config file.
...
Layer 4 - The Glance Kubernetes Operator will gather the config data passed in
          from the OpenStack Kubernetes Operator and create configmaps for the
          config files.

          The Glance Kubernetes Operator will run a Kubernetes Job to create
          the Glance DB user and password. And it will run a Kubernetes Job that
          will create a Keystone user and password.
