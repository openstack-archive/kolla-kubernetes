...
#
wait_for_virsh_vm vm-1 running

#
# Need to find the location of log file
#
sleep 30
vm_log=$(sudo find / -name vm-1.log | grep tmp | awk '{print $1}')

#
# We got the log file location, now wait 5 minutes and then dump vm log here
#
echo $vm_log
sleep 300
sudo cat $vm_log
