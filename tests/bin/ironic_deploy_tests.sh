...
function wait_for_baremetal_resources {
    ...
    while true; do
        ...
        sleep 1;
        count=$((count+1))
        [ $count -gt 120 ] && openstack hypervisor stats show && exit -1
    done
    set -x
}
...
if [ "x$base_distro" == "xubuntu" ]; then
  ...
  sudo yum install -y libvirt qemu-kvm-ev qemu-img-ev ipmitool libvirt-client libvirt-devel tcpdump
  sudo rpm -qa | sort
fi
...
#
if [ "x$base_distro" == "xubuntu" ]; then
 sudo strace sudo virsh define $DIR/../conf/ironic/vm-1.xml
fi
...
bash -c "sudo tcpdump -i tenants -vv > /tmp/packets" &
kubectl get svc -n kolla ironic-api -o yaml
sudo netstat -tunlp | grep 6385
