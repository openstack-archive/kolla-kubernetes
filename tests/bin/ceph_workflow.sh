...
helm install kolla/mariadb-pv --version 3.0.0-1 \
    --name mariadb-pv --set "element_name=mariadb,storage_provider=ceph" \
    --values <(ceph_values $1)

helm install kolla/mariadb-pvc --version 3.0.0-1 --namespace kolla \
...
helm install kolla/mariadb-job --version 3.0.0-1 \
