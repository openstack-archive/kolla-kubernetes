...
kubectl get jobs --namespace=kolla >> $WORKSPACE/logs/mariadb-job.log
kubectl get job --namespace | grep mariadb | awk '{print "kubectl describe jobs "$1" --namespace=kolla >> $WORKSPACE/logs/mariadb-job.log" | sh -l
...
helm delete mariadb-job --purge

helm delete rabbitmq-job --purge

helm install kolla/mariadb-pod --debug --version 3.0.0-1 \
