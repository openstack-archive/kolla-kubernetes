...
#FIXME force on to make progress debugging....
    ...
    ssh $USER@$line 'rpm -qi python-openstackclient'
    ssh $USER@$line 'ls ~/keystonerc_admin'
    ssh $USER@$line 'which openstack'
    ssh $USER@$line '. ~/keystonerc_admin; /usr/bin/openstack user list'
    ssh $USER@$line '. ~/keystonerc_admin; /usr/bin/openstack catalog list'
    set -e
    ssh $USER@$line '. ~/keystonerc_admin; /usr/bin/openstack server list'
done
