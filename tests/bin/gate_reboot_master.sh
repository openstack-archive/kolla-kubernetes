...
function log_kube_start_failure {
    ...
    mkdir -p "$LOGS/container/"
    ssh "$1" 'sudo docker ps -q -a --format "{{.ID}}\t{{.Names}}"' | while read line; do
        ID=$(echo $line | awk '{print $1}');
        NAME=$(echo $line | awk '{print $2}');
        ssh "$1" sudo docker logs $ID > "$LOGS/container/$NAME.txt" 2>&1;
    done
    ssh $1 'hostname' > "$WORKSPACE/logs/hostname-after.txt"
    ssh $1 'ip a' > "$WORKSPACE/logs/ip-after.txt"
}
...
cat /etc/nodepool/sub_nodes_private | while read line; do
    ...
    ssh $USER@$line 'hostname' > "$WORKSPACE/logs/hostname-before.txt"
    ssh $USER@$line 'ip a' > "$WORKSPACE/logs/ip-before.txt"
    RES=$?
    set +e
