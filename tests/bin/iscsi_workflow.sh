...
common_vars="ceph_backend true=false,enable_kube_logger=false,kolla_base_distro=$base_distro"
...
helm install kolla/cinder-volume-host --version 3.0.0-1 \
    --set "$common_vars,element_name=cinder" --namespace kolla \
    --name cinder-volume-host
...
helm install kolla/glance-api --version 3.0.0-1 \
    --set "$common_vars,host_backend=true" \
...
kubectl create -f $DIR/tests/conf/iscsid-tgtd-configmap.yaml
