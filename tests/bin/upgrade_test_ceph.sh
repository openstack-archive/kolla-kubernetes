...
pushd /tmp
helm upgrade test-ceph-mon-daemonset kolla/test-ceph-mon-daemonset --version $VERSION \
    --namespace kolla \
...
helm upgrade ceph-osds0 kolla/test-ceph-osd-pod --version $VERSION \
    --namespace kolla \
    --values /tmp/general_config.yaml \
    --values /tmp/ceph_config.yaml \
    --set index=0
helm upgrade ceph-osds1 kolla/test-ceph-osd-pod --version $VERSION \
    --namespace kolla \
    --values /tmp/general_config.yaml \
    --values /tmp/ceph_config.yaml \
    --set index=1
