...
RUN apt-get update && apt-get -y install python-dev curl libffi-dev gcc libssl-dev sshpass wget crudini git
...
RUN git clone http://github.com/openstack/kolla-ansible /kolla-ansible
RUN pip install -U /kolla-ansible/ /kolla-kubernetes/
RUN cp -a /kolla-ansible/etc/kolla /etc
RUN cp -a /kolla-kubernetes /etc